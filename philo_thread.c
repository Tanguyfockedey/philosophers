...
/*   Updated: 2024/03/28 19:50:25 by tafocked         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	philo_live(t_philosopher *philo)
{
	...
	if (pthread_mutex_lock(&philo->rules->eating))
		return (err_msg(1, "Eating lock fail !"));
	philo->time_last_eat = timestamp();
	print_action(philo, "is eating");
	if (pthread_mutex_unlock(&philo->rules->eating))
		return (err_msg(1, "Eating unlock fail !"));
	philo->nb_eat++;
